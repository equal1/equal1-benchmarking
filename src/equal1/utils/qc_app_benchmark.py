...
benchmark_dir = '../../thirdparty/QC-App-Oriented-Benchmarks'

def init_benchmark_environmet():
    sys.path.insert(1, f'{benchmark_dir}/_common')
    sys.path.insert(1, f'{benchmark_dir}/_common/qiskit')
...
class Equal1Executor():
    ...
    def __call__(self, qc : QuantumCircuit, backend_name : str, backend, shots, **kwargs) -> Equal1Result:
        ...
        job = self.device.run(qc, shots=shots, noise_model=self.hardware, runtime_options=runtime_options, backend=backend)
